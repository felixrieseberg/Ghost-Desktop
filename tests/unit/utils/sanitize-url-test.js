...
test('adds a trailing slash', function(assert) {
    ...
    assert.equal(result, 'http://testblog.com/');
});
...
test('removes a trailing /ghost/', function(assert) {
...
test('does not remove a middleing /ghost/', function(assert) {
    ...
    assert.equal(result, 'http://testblog.com/ghost/myblog/');
});

test('does not remove a middleing ghost', function(assert) {
    let result = sanitizeUrl('http://testblog.com/ghost-blogs/myblog/');
    assert.equal(result, 'http://testblog.com/ghost-blogs/myblog/');
});

test('adds a http:// if required', function(assert) {
    let result = sanitizeUrl('testblog.com/ghost-blogs/myblog/');
    assert.equal(result, 'http://testblog.com/ghost-blogs/myblog/');
});

test('does correctly identify https as not-to-be-replaced', function(assert) {
    let result = sanitizeUrl('https://testblog.com/ghost-blogs/myblog/');
    assert.equal(result, 'https://testblog.com/ghost-blogs/myblog/');
});